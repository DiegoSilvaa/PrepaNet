...
import CardMedia from '@mui/material/CardMedia';
  ...
  export default function Dashbord() {
    ...
    const baseUrl = 'https://prepnet.uc.r.appspot.com/api/coords/home/';
    const baseUrlPeriod = 'https://prepnet.uc.r.appspot.com/api/coords/periods/';
    ...
    useEffect(() => {
      ...
      axios.get(`${baseUrl}`, { headers: {"Content-Type" : 'application/json',"x-auth-token": authCTX.token}})
        .then((response) => {
          ...
          response.data.promedio.map(item => {
              //console.log("item", item)
              taller.push(item.name);
              promedio.push(item.promedio)
          })
          setTotal(response.data);
          setnameTaller(taller)
          setData(promedio)
          setIsSub(true);
    ...
    <Sidenav/>
        ...
        <Box sx={{height: '10%', width:'100%', bgcolor: '#146ca4', mt: 2,borderRadius: 1, color: "white", justifyContent:"center",alignItems:"center", display:"flex"}}>
          <Typography variant="h5"> Panel de Coordinador </Typography> 
          </Box>
          <Stack direction="row" spacing={'5%'} mt={2} justifyContent="center">
          <Box sx={{height: '40%', width:'50%', borderRadius: 1, color: "white", justifyContent:"center",alignItems:"center"}}>
          <Stack spacing={'5%'}>
          ...
          <Box sx={{width: "60%",height: "100%",backgroundColor: 'white',border: 1, borderColor: 'black',justifyContent:"center",alignItems:"center"}}>
              <Card sx={{ maxWidth: "100%" }}>
